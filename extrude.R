...
extrude <- function(x, var, k = 0.5, col = "red", add = FALSE) {
  ...
  for (i in 1:nrow(x)){
    ...
    nb <- dim(pts)[1] - 1
      ...
      if (z == 1){
        faces <- st_sf(pos = pos, ang = ang, fill = fill[2], border = border[2], geometry=st_sfc(st_polygon(list(rbind(dot1, dot2, dot3, dot4, dot1)))))
      } else {
        face <- st_sf(pos = pos, ang = ang, fill = fill[2], border = border[2], geometry=st_sfc(st_polygon(list(rbind(dot1, dot2, dot3, dot4, dot1)))))
...
extrude(x, "pop2019", k = 0.01, col ="#f0a800",add=T)


# TEST


polys <- x[x$id %in% c("TX","LA"),]
View(x)
